...
# popups are opaque (blur is off for cheap flicker-free repaints, so
# translucency would let window content bleed through)
POPUP_BG_SOLID="0xff${POPUP_BG#0x??}"

# shared popup styling (word-split on purpose, values contain no spaces)
export POPUP_PROPS="popup.background.color=$POPUP_BG_SOLID \
